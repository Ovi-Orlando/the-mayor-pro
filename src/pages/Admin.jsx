...
export default function Admin(){
  ...
  if(!allowed){
    return <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#000",color:"#fff"}}><div>Acceso restringido.</div></div>
  }
    ...
    <div style={{minHeight:"100vh",background:"#000",color:"#fff",padding:20}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:12}}>
          <button onClick={add} style={{background:"#e50914",border:"none",padding:"8px 12px",borderRadius:8}}>+ Agregar</button>
          ...
          <div key={it.id} style={{background:"#0b0b0b",padding:12,borderRadius:8,display:"grid",gridTemplateColumns:"120px 1fr 120px",gap:12,alignItems:"center",marginBottom:12}}>
            ...
            <div>
              ...
              <div style={{display:"flex",gap:8}}>
                <input className="input" value={it.genero||""} onChange={e=>update(idx,"genero",e.target.value)} placeholder="Género" style={{flex:1}} />
                <input className="input" value={it.anio||""} onChange={e=>update(idx,"anio",e.target.value)} placeholder="Año" style={{width:100}} />
              </div>
              <input className="input" value={it.descripcion||""} onChange={e=>update(idx,"descripcion",e.target.value)} placeholder="Descripción" style={{width:"100%",marginTop:8}} />
              <input className="input" value={it.imagen||""} onChange={e=>update(idx,"imagen",e.target.value)} placeholder="URL imagen" style={{width:"100%",marginTop:8}} />
              <input className="input" value={it.video||""} onChange={e=>update(idx,"video",e.target.value)} placeholder="URL video (mp4)" style={{width:"100%",marginTop:8}} />
            </div>
